...
def _robot_request(base_url, path, params=None, method="GET"):
    """Request к роботу. base_url без http, например 192.168.4.1."""
    if not base_url:
    ...
    if method == "POST" and params:
        # For POST requests to robotProgrammm.ino, send as form data
...
def _execute_commands(commands, base_url):
    """Executes list of commands (turn, drive, lift) via HTTP for robotProgrammm.ino."""
    for cmd_type, kwargs in commands:
        if cmd_type == "turn":
            # robotProgrammm.ino doesn't have turn endpoint, skip for now
            continue
        elif cmd_type == "drive":
            # Convert drive command to GO_LOCAL format
            target_node = kwargs.get("target_node")
            if target_node:
                # Get node coordinates from graph (need to pass graph)
                _, err = _robot_request(base_url, "/command", {"commands": f"GO_LOCAL {target_node} 0"}, method="POST")
            else:
                _, err = _robot_request(base_url, "/command", {"commands": f"GO_LOCAL {kwargs.get('x', 0)} {kwargs.get('y', 0)}"}, method="POST")
        elif cmd_type == "lift":
            lift_action = kwargs.get("action", "up")
            if lift_action == "up":
                ...
                err = f"Invalid lift action: {lift_action}"
        else:
            continue
        if err:
            return False, err
